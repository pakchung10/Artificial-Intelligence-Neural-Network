...
def train(model, train_loader, criterion, optimizer, epoch):
    ...
    plt.title('loss')
    plt.savefig('Temperature_Train_loss_DNN.jpg', dpi=256)
    plt.close()
...
if __name__ == '__main__':
    ...
    # 数据从1901到1990
    train_loader = DataLoader(td.TemperatureDataset("../../meant_1901to2000.csv", mode="train", testset_ratio=0.1),
    ...
    # # 保存模型
    # print("训练完成")
    # torch.save(model, "temperature_model_DNN.pth")
    # print("模型已保存")

    # 测试模型
    model = torch.load("temperature_model_DNN.pth")
