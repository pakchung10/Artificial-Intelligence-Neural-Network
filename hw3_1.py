...
def my_perceptron(train_list, label_list, length):
    ...
    while True:
        ...
        for index in l:
            ...
            CheckPoint = w_[0] * data[index][0] + w_[1] * data[index][1] + w_[-1]
            print("测试值：", CheckPoint)
            print("实际值：", label[index])
            if CheckPoint * label[index] <= 0:
                print("为误分点")
                w_ = [w_[0] + data[index][0] * label[index],
                      w_[1] + data[index][1] * label[index], w_[2] + label[index]]
                print("更新权重w_", w_)
            else:
                print("不为误分点")
                print("保持权重w_", w_)
